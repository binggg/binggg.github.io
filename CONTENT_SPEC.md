...
> **一个做了 10 年工程的开发者，在 AI 时代的真实踩坑记录。**

不追求高频，每篇都是想说、有东西可说才写。跟那些日更 AI 新闻的号的区别在于：**经历不可复制。**

### 四大内容支柱

| 支柱 | 占比 | 说明 | 代表话题 |
|---|---|---|---|
| **① 技术深度拆解** | 40% | 源码级分析、协议拆解、架构取舍。这是跟其他 AI 博主最大的差异点——你是做工程的，不是用 AI 写文案的。 | MCP/Agent 协议、Claude Code 机制、CloudBase AI Toolkit 架构、后端选型 |
| **② 工程方法论** | 30% | 从迅雷到腾讯，工程化这条线上十年的判断。这个角度 AI 博主写不了。 | Spec 驱动开发、反 AI 腔、AI code review、EARS 语法实践 |
| **③ 踩坑实录** | 20% | AI 不踩坑——它没有"我没想到这个会崩"的时刻。这是最不能替代的内容。 | 视频管线翻车、Agent 调度实战、CI bash set -e 陷阱 |
| **④ 工具推荐/工作流** | 10% | 仅限于自己真实用过、用够一个月的工具。没用过不写，没用够一个月不写。 | ai-ctl 管线、多 agent 调度、实际在用的 AI 工具链 |

### 不发清单

- ❌ 产品功能介绍（像 CloudBase 新功能上线——像公司 PR）
- ❌ 竞品对比分析（如 Supabase vs CloudBase——身份敏感，读者认为软文）
- ❌ 纯翻译（不是你自己的声音）
- ❌ 二手研究（没亲自用过就写，没深度）
- ❌ 副业相关内容（跟主品牌混淆）
